...
re_dt1 = /^(\d{4})\-(\d{1,2})\-(\d{1,2})$/,
re_tm = /^(\d{1,2})\:(\d{1,2})\:(\d{1,2})$/,
a_formats = {
	'alpha'   : /^[a-zA-Z\.\-]*$/,
	'number'   : /^[1-9]*$/,
	'alphanum': /^[a-z\d\-_\s]+$/i /*/^\w+$/*/,
	'unsigned': /^\d+$/,
	'year4'	  : /^(\d{4})$/,
	'integer' : /^[\+\-]?\d*$/,
	'real'    : /^[\+\-]?\d*\.?\d*$/,
	'email'   : /^[\w-\.]+\@[\w\.-]+\.[a-z]{2,4}$/,
	'phone'   : /^[\d\.\s\-]+$/,
	'date1'    : function (s_date) {
		// check format
		if (!re_dt1.test(s_date))
			return false;
		...
		for (var n_key in this.a_fields) {
			...
			if (s_message) {
				...
				if (b_dom && this.a_fields[n_key].o_tag)
					this.a_fields[n_key].o_tag.className = 'error_field';
			}
		}
...
function ProcessIndicator(action)
{
	if(action==1)
	{
		$( "#processindicator" ).css('display','block');
		$( "#innerind" ).css('display','block');
		}
	else
	{
		$( "#processindicator" ).css('display','none');
		$( "#innerind" ).css('display','none');
		}
	}
function DateFormat( date , formate )
{
	...
	return newdate.toDateString(formate);
	}
...
var vehiclecheck = {
	...
	'LicencePlate':{'l':'License Plate','r':true,'f':'alphanum','t':'LicencePlate'},
	'states':{'l':'State','r':true,'f':'alpha','t':'states'},
	'color':{'l':'Vehicle Colour','r':true,'f':'alpha','t':'color'},
	'Make':{'l':'Vehicle Maker','r':true,'f':'integer','t':'Make'},	
	'Type':{'l':'Vehicle Type','r':true,'f':'integer','t':'Type'},
	'Years':{'l':'Vehicle Type','r':false,'f':'year4','t':'Years'},
	'Terms':{'l':'Terms And Services','r':true,'t':'Terms'},
 	};

var vehicle = new validator('vehicleform', vehiclecheck, o_config);
 
// End Vehicle Validations

var permitcheck = {

	'Permit':{'l':'Permit','r':true,'f':'number','t':'Permit'},
	'StartingDate':{'l':'Starting Date','r':true,'f':'date1','t':'StartingDate'},
  
	};

var permit = new validator('RegisterPermitForm', permitcheck, o_config);
